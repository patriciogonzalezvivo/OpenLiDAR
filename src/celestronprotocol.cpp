...
void SetRate(int _newRate);
double GetRA(void);
double GetDec(void);
int SlewToCoords(double _newRA, double _newDec);
int SyncToCoords(double _newRA, double _newDec);
...
int ConnectTel(const char *_port) {
    struct termios tty;
    char returnStr[128];
    int numRead;
...
void SetRate(int _newRate) {
    ...
    else if (_newRate == FIND) {
        slewRate = 6;
    }
    else if (_newRate == CENTER) {
        slewRate = 3;
    }
    else if (_newRate == GUIDE) {
        slewRate = 1;
    }
}
...
int SyncToCoords(double _newRA, double _newDec)
{
   ...
   offsetDec = 0.;
   GetRAandDec();
   offsetRA = returnRA - _newRA;
   offsetDec = returnDec - _newDec;
   ...
   return (0);*/
    ...
    n1 = _newRA * 0x1000000 / 24;
    n2 = _newDec * 0x1000000 / 360;
    n1 = n1 << 8;
    n2 = n2 << 8;
...
int SlewToCoords(double _newRA, double _newDec) {
    ...
    if ((_newDec >= 0.0) && (_newDec <= 90.0)) {
        degs = degs * (3. * 60. * 65536. / 64800.);
    }
    else if ((_newDec < 0.0) && (_newDec >= -90.0)) {
